...
(() => {
  ...
  /* ---------------------------------------------------------
     ...
     DayRecord = {
       ...
       kmFinal: number|null,
       consumo: number|null,            // km/L definido ao "Iniciar dia"
       precoCombustivel: number|null,   // R$/L definido ao "Iniciar dia"
       gastosExtra: number,
       encerrado: boolean,
       routes: [ RouteRecord ]
     }
     RouteRecord = {
       id, km, paradas, feriado, vehicle,
       valorKm, valorParadas, faturamento,
       combustivel, lucro, consumoUsado, precoUsado, createdAt
     }
  --------------------------------------------------------- */

  function novoDayRecord() {
    return {
      kmInicial: null,
      kmFinal: null,
      consumo: null,
  ...
  // Encontra o consumo/preço mais recente já preenchido em algum dia
  // anterior (ou no próprio dia de hoje), para servir de valor padrão
  // quando o motorista cria uma rota sem ter clicado em "Iniciar dia".
  function getLastKnownFuelDefaults() {
    const keys = Object.keys(state.days).sort().reverse();
    for (const key of keys) {
      const d = state.days[key];
      if (d && d.consumo != null && d.precoCombustivel != null) {
        return { consumo: d.consumo, precoCombustivel: d.precoCombustivel };
      }
    }
    // Nenhum dia preenchido ainda: fallback de fábrica razoável.
    return { consumo: 10, precoCombustivel: 5.89 };
  }

  // Retorna o consumo/preço efetivos do dia: usa o que foi definido em
  // "Iniciar dia"; se ainda não foi definido, usa o último valor conhecido.
  function getFuelInputsForDay(dayKey) {
    const day = state.days[dayKey];
    if (day && day.consumo != null && day.precoCombustivel != null) {
      return { consumo: day.consumo, precoCombustivel: day.precoCombustivel };
    }
    return getLastKnownFuelDefaults();
  }
  ...
  // 5.3 Combustível estimado de uma rota específica, usando o consumo e
  // preço do combustível definidos no "Iniciar dia" (ou o último valor
  // conhecido, se o dia ainda não foi iniciado).
  ...
  // 5.4 Cálculo completo de uma rota: faturamento, combustível e lucro.
  function calcRoute({ km, paradas, feriado, vehicle, consumo, precoCombustivel }) {
    ...
    return { valorKm, valorParadas, faturamento, combustivel, lucro };
  }

  // 5.5 Totais de um dia: soma o faturamento, combustível e lucro de cada
  // rota já calculados individualmente (cada rota pode ter usado um
  // consumo/preço diferente, caso o motorista tenha reiniciado o dia).
  ...
  // 5.6 Quinzena: 15 dias corridos a partir de inicioQuinzena,
  ...
  const els = {
    ...
    diaConsumoInput: $('#dia-consumo-input'),
    diaPrecoInput: $('#dia-preco-input'),
  ...
  function renderKmRow(day) {
    ...
    if (!day || day.kmInicial == null) {
      ...
      btn.id = 'btn-iniciar-dia-dynamic';
      btn.textContent = 'Iniciar dia';
      btn.addEventListener('click', openIniciarDiaModal);
    ...
    const consumoLabel = day.consumo != null ? `${day.consumo} km/L` : '—';
    const precoLabel = day.precoCombustivel != null ? fmtBRL(day.precoCombustivel) + '/L' : '—';
    summary.innerHTML = `
    ...
    fuelInfo.className = 'km-summary';
    fuelInfo.innerHTML = `
      <span>Consumo hoje: <b>${consumoLabel}</b></span>
      <span>Combustível hoje: <b>${precoLabel}</b></span>
    `;
    els.kmRow.appendChild(fuelInfo);
  }
  ...
  function updateRoutePreview() {
    ...
    const { consumo, precoCombustivel } = getFuelInputsForDay(todayKey());

    const { valorKm, valorParadas, faturamento, combustivel, lucro } =
      calcRoute({ km, paradas, feriado, vehicle, consumo, precoCombustivel });
  ...
  els.btnAddRoute.addEventListener('click', () => {
    ...
    updateRoutePreview();
    openModal(els.modalRoute, () => els.routeKm.focus());
  });
  ...
  els.btnSaveRoute.addEventListener('click', () => {
    ...
    const vehicle = state.settings.vehicle;
    const key = todayKey();
    const { consumo, precoCombustivel } = getFuelInputsForDay(key);
    const { valorKm, valorParadas, faturamento, combustivel, lucro } =
      calcRoute({ km, paradas, feriado, vehicle, consumo, precoCombustivel });
    ...
    day.routes.push({
      id: uid(),
      km, paradas, feriado, vehicle,
      valorKm, valorParadas, faturamento,
      combustivel, lucro,
      consumoUsado: consumo, precoUsado: precoCombustivel,
      createdAt: Date.now()
    });
    persistDays();
  ...
  /* ---------------------------------------------------------
     11. MODAL: INICIAR DIA / KM FINAL
  --------------------------------------------------------- */

  // Abre o modal "Iniciar dia" já com os campos de consumo/preço
  // pré-preenchidos com o último valor conhecido, para o motorista só
  // precisar ajustar se algo tiver mudado.
  function openIniciarDiaModal() {
    const key = todayKey();
    const day = getDay(key);
    const defaults = getFuelInputsForDay(key);

    els.kmInicialInput.value = day.kmInicial != null ? day.kmInicial : '';
    els.diaConsumoInput.value = day.consumo != null ? day.consumo : defaults.consumo;
    els.diaPrecoInput.value = day.precoCombustivel != null ? day.precoCombustivel : defaults.precoCombustivel;

    openModal(els.modalKmStart, () => els.kmInicialInput.focus());
  }
  ...
  els.btnSaveKmStart.addEventListener('click', () => {
    ...
    const consumo = clampNum(els.diaConsumoInput.value, NaN);
    const precoCombustivel = clampNum(els.diaPrecoInput.value, NaN);
    ...
    if (!Number.isFinite(consumo) || consumo <= 0) {
      showToast('Informe o consumo do veículo hoje');
      els.diaConsumoInput.focus();
      return;
    }
    if (!Number.isFinite(precoCombustivel) || precoCombustivel <= 0) {
      showToast('Informe o preço do combustível hoje');
      els.diaPrecoInput.focus();
      return;
    }
    ...
    day.kmInicial = km;
    day.consumo = consumo;
    day.precoCombustivel = precoCombustivel;
    persistDays();
  ...
  els.btnFinishDay.addEventListener('click', () => {
    ...
    if (day.kmInicial == null) {
      showToast('Inicie o dia primeiro');
      return;
    }
  ...
  els.btnAddFuel.addEventListener('click', () => {
    ...
    // Se o dia de hoje já foi iniciado, atualiza o preço do combustível
    // do dia com o valor mais recente informado no abastecimento — assim
    // as próximas rotas do dia já usam esse preço atualizado.
    const todayDay = state.days[todayKey()];
    if (todayDay && todayDay.consumo != null) {
      todayDay.precoCombustivel = preco;
      persistDays();
    }
