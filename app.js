...
(() => {
  ...
  /* ---------------------------------------------------------
     ...
     RouteRecord = {
       ...
       valorKm, valorParadas, total, createdAt
     }
  --------------------------------------------------------- */
  ...
  // 5.3 Cálculo completo de uma rota
  function calcRoute({ km, paradas, feriado, vehicle }) {
    const valorKm = calcValorKm(vehicle, km);
    const valorParadas = calcValorParadas(paradas);
    let total = valorKm + valorParadas;
    if (feriado) total *= FERIADO_MULTIPLIER;
    return { valorKm, valorParadas, total };
  }

  // 5.4 Combustível estimado para um KM rodado
  function calcCombustivel(kmRodado) {
    const consumo = clampNum(state.settings.consumo, 0);
    const preco = clampNum(state.settings.precoCombustivel, 0);
    if (consumo <= 0) return 0;
    const litros = kmRodado / consumo;
    return litros * preco;
  }

  // 5.5 Totais de um dia
  function calcDayTotals(dayKey) {
    ...
    const faturamento = day.routes.reduce((acc, r) => acc + r.total, 0);
    ...
    const combustivel = calcCombustivel(kmRodado);
    const lucro = faturamento - combustivel - gastosExtra;

    return { faturamento, kmRodado, combustivel, lucro, rotas, gastosExtra };
  }
  ...
  const els = {
    ...
    cfgConsumo: $('#cfg-consumo'),
    cfgPreco: $('#cfg-preco-combustivel'),
  ...
  function updateRoutePreview() {
    ...
    const { valorKm, valorParadas, total } = calcRoute({ km, paradas, feriado, vehicle });
    ...
    els.prevTotalVal.textContent = fmtBRL(total);
  }
  ...
  els.btnSaveSettings.addEventListener('click', () => {
    ...
    state.settings.consumo = clampNum(els.cfgConsumo.value, 10) || 10;
    state.settings.precoCombustivel = clampNum(els.cfgPreco.value, 5.89);
